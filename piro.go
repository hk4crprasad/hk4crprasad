...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present PIRO will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("PIROMAXPROCS")
	...
	go func() {
		fmt.Println("-- PIRO Attack Started --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- PIRO Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
